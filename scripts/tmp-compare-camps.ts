// TEMP: verify the frontier camp ports match the expansion SVG exports exactly.
import { readFileSync } from "node:fs";
import { makeDrownedRuins, makeBarrowCrypt, makeAshenWarcamp } from "../game/render/sprites";
...
const ref = (name: string) =>
  readFileSync(`public/assets/expansion.nosync/_naevyr_expansion_pkg/camps/${name}.svg`, "utf8").trim();
...
function diff(name: string, frames: Grid[]) {
  ...
  if (mine === ref(name)) console.log(`OK   ${name}`);
    ...
    fail++;
    console.error(`DIFF ${name}: mine ${mine.length}b vs ref ${ref(name).length}b`);
    const r = ref(name);
    for (let i = 0; i < Math.min(mine.length, r.length); i++) if (mine[i] !== r[i]) {
      console.error(` first diff @${i}: mine …${mine.slice(i - 30, i + 30)}… ref …${r.slice(i - 30, i + 30)}…`); break;
    }
  }
}
diff("drowned_ruins", [0, 1].map((f) => G(makeDrownedRuins(f))));
diff("barrow_crypt", [0, 1].map((f) => G(makeBarrowCrypt(f))));
diff("ashen_warcamp", [0, 1].map((f) => G(makeAshenWarcamp(f))));
